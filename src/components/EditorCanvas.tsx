...
export const EditorCanvas = ({ strategyId }: EditorCanvasProps) => {
    ...
    const handleDuplicateStep = () => {
        ...
        const newStep = { id: Date.now().toString(), name: `${currentStep.name} (Copy)`, data: clonedData };
        setSteps(prev => { const newSteps = [...prev]; newSteps.splice(currentStepIndex + 1, 0, newStep); return newSteps; });
        setCurrentStepIndex(prev => prev + 1);
    };
    ...
    const handleDeleteStep = (index: number) => {
        ...
        if (currentStepIndex >= index && currentStepIndex > 0) setCurrentStepIndex(prev => prev - 1);
    };
    ...
    // --- INITIALISATION ---
    useEffect(() => {
        const fetchFolders = async () => { const { data } = await supabase.from('folders').select('id, name'); if (data) setFolders(data); };
        fetchFolders();
    }, []);
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            if (data) {
                ...
                if (Array.isArray(rawData)) { setSteps([{ id: 'init', name: 'Setup', data: rawData }]); setCurrentStepIndex(0); }
                else if (rawData && rawData.steps) { setSteps(rawData.steps); setCurrentStepIndex(rawData.currentStepIndex || 0); }
            }
            setIsLoaded(true);
        };
        loadInitialData();
    ...
    const { processPendingUpdates } = useRealtimeStrategy(
        ...
        (newDrawings: any) => {
            if (Array.isArray(newDrawings)) {
                setSteps(prev => { const newSteps = [...prev]; newSteps[currentStepIndex] = { ...newSteps[currentStepIndex], data: newDrawings }; return newSteps; });
    ...
    const debouncedSave = useMemo(() => debounce((id: string, stepsData: StrategyStep[], idx: number) => { const payload = { steps: stepsData, currentStepIndex: idx }; updateStrategyData(id, payload as any); }, 1000), [updateStrategyData]);
    useEffect(() => { if (!isLoaded || !strategyId) return; if (isRemoteUpdate.current) { isRemoteUpdate.current = false; return; } debouncedSave(strategyId, steps, currentStepIndex); return () => debouncedSave.cancel(); }, [steps, currentStepIndex, strategyId, isLoaded, debouncedSave]);

    // --- HANDLERS FICHIERS/MODALES ---
    const handleFolderChange = async (newFolderId: string) => { setCurrentFolderId(newFolderId); if (strategyId) await supabase.from('strategies').update({ folder_id: newFolderId || null }).eq('id', strategyId); };
    const handleDeleteRequest = () => setShowDeleteModal(true);
    const confirmDelete = async () => { if (!strategyId) return; const { error } = await supabase.from('strategies').delete().eq('id', strategyId); if (!error) navigate('/'); else console.error("Erreur suppression:", error); };
    const handleLoadStrategy = (strat: any) => { if (strat && strat.data) { if (Array.isArray(strat.data)) { setSteps([{ id: 'init', name: 'Setup', data: strat.data }]); setCurrentStepIndex(0); } else if (strat.data.steps) { setSteps(strat.data.steps); setCurrentStepIndex(strat.data.currentStepIndex || 0); } } setShowLoadModal(false); };
    useEffect(() => { if (currentTool === 'text') setIsTextModalOpen(true); }, [currentTool]);

    // --- HELPERS GRAPHIQUES (AVEC ROTATION) ---
    ...
    const redrawMainCanvas = useCallback(() => {
        ...
        // @ts-ignore
        // On passe isRotated pour que les icones/textes restent à l'endroit
    ...
    const handleSaveText = (data: { text: string; color: string; fontSize: number; isBold: boolean; isItalic: boolean }) => {
        if (editingTextId !== null) {
            ...
            // Calcul ajusté pour la rotation
            ...
            const newText: DrawingObject = { id: Date.now(), tool: 'text', text: data.text, points: [], x: mapX, y: mapY, color: data.color, fontSize: data.fontSize, fontWeight: data.isBold ? 'bold' : 'normal', fontStyle: data.isItalic ? 'italic' : 'normal', thickness: 0, opacity: 1 };
            updateDrawings(prev => [...prev, newText]);
        }
        setIsTextModalOpen(false); setCurrentTool('cursor');
    };
    ...
    const handleDoubleClick = (e: React.MouseEvent) => {
        ...
        for (let i = drawings.length - 1; i >= 0; i--) {
            ...
            if (obj.tool === 'text' && obj.x !== undefined && obj.y !== undefined) {
                const fontSize = obj.fontSize || 20; const approxWidth = (obj.text?.length || 0) * (fontSize * 0.6);
                if (Math.abs(pos.x - obj.x) < approxWidth/2 && Math.abs(pos.y - obj.y) < fontSize/2) { setEditingTextId(obj.id); setIsTextModalOpen(true); return; }
            }
        }
    };
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        const mapScale = getCurrentScale();
        const pos = getMousePos(e); // Utilise les coordonnées ajustées
        ...
        if (currentTool === 'cursor'|| currentTool === 'settings' || currentTool === 'tools') {
            for (let i = drawings.length - 1; i >= 0; i--) {
                ...
                if (obj.tool === 'text' && obj.x !== undefined && obj.y !== undefined) {
                    const fontSize = obj.fontSize || 20; const approxWidth = (obj.text?.length || 0) * (fontSize * 0.6);
                    if (Math.abs(pos.x - obj.x) < approxWidth/2 && Math.abs(pos.y - obj.y) < fontSize/2) { setDraggingObjectId(obj.id); setDragOffset({ x: pos.x - obj.x, y: pos.y - obj.y }); return; }
                }
                if (obj.tool === 'image' && obj.x != null && obj.y != null) {
                    const w = (obj.width || iconSize) * mapScale; const h = (obj.height || iconSize) * mapScale;
                    if (pos.x >= obj.x - w/2 && pos.x <= obj.x + w/2 && pos.y >= obj.y - h/2 && pos.y <= obj.y + h/2) { setDraggingObjectId(obj.id); setSpecialDragMode(null); setDragOffset({ x: pos.x - obj.x, y: pos.y - obj.y }); return; }
                }
                // @ts-ignore
                const hit = checkAbilityHit(pos, obj, mapScale);
                if (hit) { setDraggingObjectId(hit.id); setSpecialDragMode(hit.mode); if (hit.offset) setDragOffset(hit.offset); if (hit.centerStart) setWallCenterStart(hit.centerStart); return; }
            }
            setDraggingObjectId(null); return;
        }
        if (currentTool === 'agent') { updateDrawings(prev => [...prev, { id: Date.now(), tool: 'image', subtype: 'agent', points: [], color: '#fff', thickness: 0, opacity: 1, imageSrc: selectedAgent, x: pos.x, y: pos.y, width: iconSize, height: iconSize }]); return; }
        if (currentTool === 'eraser') { setIsDrawing(true); eraseObjectAt(pos.x, pos.y); return; }
        if (currentTool === 'pen') { setIsDrawing(true); startPosRef.current = { x: pos.x, y: pos.y }; pointsRef.current = [{ x: pos.x, y: pos.y }]; const tempCtx = getContext(tempCanvasRef); if (tempCtx) { tempCtx.clearRect(0, 0, tempCtx.canvas.width, tempCtx.canvas.height); tempCtx.beginPath(); tempCtx.moveTo(pos.x, pos.y); } }
    };

    const handleMouseMove = (e: React.MouseEvent) => {
        if (draggingObjectId !== null && trashRef.current) { const trashRect = trashRef.current.getBoundingClientRect(); const isOver = e.clientX >= trashRect.left && e.clientX <= trashRect.right && e.clientY >= trashRect.top && e.clientY <= trashRect.bottom; setIsOverTrash(isOver); } else if (isOverTrash) { setIsOverTrash(false); }
        ...
        const pos = getMousePos(e); // Ajusté
        const canvas = mainCanvasRef.current;
        if (canvas) { pos.x = clamp(pos.x, 0, canvas.width); pos.y = clamp(pos.y, 0, canvas.height); }

        if (draggingObjectId !== null) {
            updateDrawings(prev => prev.map(obj => {
                if (obj.id !== draggingObjectId) return obj;
                if ((obj.tool === 'image' || obj.tool === 'text') && obj.x !== undefined) { return { ...obj, x: pos.x - dragOffset.x, y: pos.y - dragOffset.y }; }
                // @ts-ignore
                return updateAbilityPosition(obj, pos, specialDragMode, dragOffset, wallCenterStart, getCurrentScale());
            })); return;
        }
        if (!isDrawing) return;
        ...
        if (currentTool === 'pen') {
            ...
            else { drawSmoothLine(tempCtx, pointsRef.current); if ((strokeType === 'arrow' || strokeType === 'dashed-arrow') && pointsRef.current.length > 2) { const last = pointsRef.current[pointsRef.current.length - 1]; const prev = pointsRef.current[Math.max(0, pointsRef.current.length - 5)]; if(prev && last) drawArrowHead(tempCtx, prev.x, prev.y, last.x, last.y, thickness); } }
        }
    };

    const handleMouseUp = () => {
        if (draggingObjectId !== null && isOverTrash) { updateDrawings(prev => prev.filter(obj => obj.id !== draggingObjectId)); setIsOverTrash(false); setDraggingObjectId(null); setSpecialDragMode(null); setDragOffset({ x: 0, y: 0 }); isInteractingRef.current = false; processPendingUpdates(); return; }
        if (isPanning) { setIsPanning(false); if(containerRef.current) containerRef.current.style.cursor = currentTool === 'cursor' ? 'default' : (currentTool ? 'crosshair' : 'grab'); }
        setDraggingObjectId(null); setSpecialDragMode(null); setDragOffset({ x: 0, y: 0 }); setIsOverTrash(false);
        if (isDrawing) {
            setIsDrawing(false);
            if (currentTool === 'pen') {
                updateDrawings(prev => [...prev, { id: Date.now(), tool: strokeType, points: strokeType === 'rect' ? [startPosRef.current, pointsRef.current[pointsRef.current.length - 1]] : [...pointsRef.current], color: color, thickness: thickness, opacity: opacity }]);
                const tempCtx = getContext(tempCanvasRef); if (tempCtx) tempCtx.clearRect(0, 0, tempCtx.canvas.width, tempCtx.canvas.height);
            }
            pointsRef.current = [];
        }
        isInteractingRef.current = false; processPendingUpdates();
    };

    const handleMouseLeave = () => { handleMouseUp(); isInteractingRef.current = false; setIsOverTrash(false); };
    const handleContextMenu = (e: React.MouseEvent) => {
        ...
        for (let i = newDrawings.length - 1; i >= 0; i--) {
            const obj = newDrawings[i]; let hit = false;
            if (obj.tool === 'image' && obj.x != null && obj.y != null) { const w = (obj.width || iconSize) * mapScale; const h = (obj.height || iconSize) * mapScale; if (pos.x >= obj.x - w/2 && pos.x <= obj.x + w/2 && pos.y >= obj.y - h/2 && pos.y <= obj.y + h/2) hit = true; }
            else if (obj.tool === 'text' && obj.x != null) { const fontSize = obj.fontSize || 20; if (Math.abs(pos.x - obj.x) < 50 && Math.abs(pos.y - obj.y!) < fontSize) hit = true; }
            else if (['pen', 'dashed', 'arrow', 'dashed-arrow'].includes(obj.tool)) { if (obj.points.some(p => Math.hypot(p.x - pos.x, p.y - pos.y) < (obj.thickness || 4) + 10)) hit = true; }
    ...
    const getCursorStyle = () => { if (isPanning) return 'grabbing'; if (currentTool === 'cursor' || currentTool === 'tools' || currentTool === 'settings') return 'default'; if (currentTool === 'eraser') return 'crosshair'; return 'crosshair'; };
    const editingObj = editingTextId ? drawings.find(d => d.id === editingTextId) : null;

    // Logique affichage image (Fallback intelligent)
    const showReverseImage = isRotated && reverseMapSrc && !reverseMapError;
    const showReverseCalls = isRotated && reverseCallsMapSrc && !reverseCallsError;
        ...
        <div className="flex flex-col lg:flex-row h-full w-full bg-[#121212]">
            <div className="absolute top-4 left-4 z-30 lg:static lg:h-full lg:w-auto lg:p-4 lg:bg-[#181b1e] lg:border-r lg:border-gray-800">
                <ToolsSidebar
                    ...
                    strokeType={strokeType} setStrokeType={setStrokeType} color={color} setColor={setColor} opacity={opacity} setOpacity={setOpacity} thickness={thickness} setThickness={setThickness} selectedAgent={selectedAgent} setSelectedAgent={setSelectedAgent} onSave={() => {}} onLoad={fetchStrategies} showZones={showZones} setShowZones={setShowZones} iconSize={iconSize} setIconSize={setIconSize} folders={folders} currentFolderId={currentFolderId} onFolderChange={handleFolderChange} onDeleteStrategy={handleDeleteRequest} onClearAll={handleClearAll}
                    isRotated={isRotated} setIsRotated={setIsRotated}
                    showMapCalls={showMapCalls} setShowMapCalls={setShowMapCalls}
                />
            </div>

            <div ref={containerRef} className="relative flex-1 w-full h-full overflow-hidden select-none" style={{ cursor: getCursorStyle() }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseLeave} onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = "copy"; }} onDrop={handleDrop} onContextMenu={handleContextMenu} onDoubleClick={handleDoubleClick}>
                        ...
                        {currentMapSrc && (
                            <img
                                ...
                                style={{
                                    width: '100%',
                                    minWidth: '1024px',
                                    // Si on utilise l'image reverse, pas de rotation CSS. Sinon, rotation CSS.
                                    transform: (!showReverseImage && isRotated) ? 'rotate(180deg)' : 'none'
                                }}
                                onLoad={syncCanvasSize}
                                onError={() => { if (showReverseImage) setReverseMapError(true); }}
                            />
                        )}

                        {/* 2. CALQUE CALLS (SUPERPOSÉ) */}
                        {showMapCalls && (showReverseCalls || callsMapSrc) && (
                            <img
                                src={showReverseCalls ? reverseCallsMapSrc : callsMapSrc}
                                alt="Map Calls"
                                draggable={false}
                                ...
                                style={{
                                    width: '100%',
                                    minWidth: '1024px',
                                    // Idem : si image reverse, pas de rotation CSS
                                    transform: (!showReverseCalls && isRotated) ? 'rotate(180deg)' : 'none'
                                }}
                                onError={() => {
                                    if (showReverseCalls) setReverseCallsError(true);
                                    else console.log("Pas de calque calls trouvé");
                                }}
                            />
                        )}

                        {/* 3. CANVAS WRAPPER (TOUJOURS ROTATÉ EN MODE REVERSE) */}
                ...
                <StepsBar steps={steps} currentIndex={currentStepIndex} onStepChange={setCurrentStepIndex} onAddStep={handleAddStep} onDuplicateStep={handleDuplicateStep} onDeleteStep={handleDeleteStep} onRenameStep={handleRenameStep} />
                ...
                <TextEditorModal isOpen={isTextModalOpen} onClose={() => { setIsTextModalOpen(false); setEditingTextId(null); setCurrentTool('cursor'); }} onSave={handleSaveText} initialText={editingObj?.text || ''} initialColor={editingObj?.color || color} initialFontSize={editingObj?.fontSize || 24} initialBold={editingObj?.fontWeight === 'bold'} initialItalic={editingObj?.fontStyle === 'italic'} />
                <ConfirmModal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} onConfirm={confirmDelete} title="Supprimer la stratégie" message="Êtes-vous sûr de vouloir supprimer cette stratégie ? Cette action est irréversible." isDangerous={true} confirmText="Supprimer" />
            </div>
        </div>
    );
};